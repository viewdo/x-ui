...
In our examples above, we discussed routes and sub-routes. The routes are expressed in our Experience UI using the [\<x-app-view\>](/components/x-app-view) tag and their sub-routes would be [\<x-app-view-do\>](/components/x-app-view-do) tags.
