...
export async function getRemoteContent(win: Window, src: string, mode: RequestMode, tokens: boolean) {
  ...
  throw new Error(`Request to ${resolvedSrc} was not successful: ${response.statusText}`)
}
